...
final class DemoLocalization {
    ...
    private func zhHansText(_ key: DemoTextKey) -> String {
        ...
        case .popToTargetLevelOneMessage: return "这是同 route 不同参数的目标页面。A 页面会先压入 B 页面，B 页面再进入第二级。"
        case .popToTargetLevelOneButton: return "继续测试精确 PopTo"
        case .popToTargetLevelTwoTitle: return "指定 Pop 第二级"
        case .popToTargetLevelTwoMessage: return "点击按钮调用 router.popTo(path:params:)，应跳过 B 页并精确回到 A 页。"
    ...
    private func englishText(_ key: DemoTextKey) -> String {
        ...
        case .popToTargetLevelOneMessage: return "This target uses the same route with different params. Page A pushes page B, then B opens level two."
        case .popToTargetLevelOneButton: return "Continue precise PopTo"
        case .popToTargetLevelTwoTitle: return "Pop Target Level Two"
        case .popToTargetLevelTwoMessage: return "Tap the button to call router.popTo(path:params:). It should skip B and return exactly to A."
